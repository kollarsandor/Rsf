...
const RSFCore = struct {
    ...
    f16_buf: ?[]f16,
    gpu_layer_weights: ?[]accel.GPULayerWeights,
};
...
fn freeGPULayerWeights(core: *RSFCore) void {
    if (core.gpu_layer_weights) |lws| {
        if (core.gpu_accel) |*ga| {
            for (lws) |*lw| lw.deinit(&ga.ctx);
        }
        core.allocator.free(lws);
        core.gpu_layer_weights = null;
    }
}

fn destroyModelCore(core: *RSFCore) void {
    freeGPULayerWeights(core);
    if (core.gpu_accel) |*ga| {
        ga.deinit();
        core.gpu_accel = null;
    }
...
fn modelGPUCompatible(core: *const RSFCore) bool {
    ...
    if (core.num_layers == 0 or core.layers.len == 0) return false;
...
fn syncAllLayersGPU(core: *RSFCore) !void {
    ...
    try staged_accel.setWeightsS(core.layers[0].s_weight.data, core.dim, core.dim);
    try staged_accel.setWeightsT(core.layers[0].t_weight.data, core.dim, core.dim);
    try staged_accel.setBiasS(core.layers[0].s_bias.data);
    try staged_accel.setBiasT(core.layers[0].t_bias.data);

    freeGPULayerWeights(core);

    const layer_weights = core.allocator.alloc(accel.GPULayerWeights, core.num_layers) catch return error.NoGPUAvailable;
    var initialized: usize = 0;
    errdefer {
        var j: usize = 0;
        while (j < initialized) : (j += 1) layer_weights[j].deinit(&staged_accel.ctx);
        core.allocator.free(layer_weights);
    }

    for (core.layers, 0..) |*layer, i| {
        layer_weights[i] = accel.GPULayerWeights.initFromCPU(
            &staged_accel.ctx,
            layer.s_weight.data,
            layer.t_weight.data,
            layer.s_bias.data,
            layer.t_bias.data,
        ) catch return error.NoGPUAvailable;
        initialized += 1;
    }
    ...
    core.gpu_layer_weights = layer_weights;
    core.f16_buf = null;
...
fn tryForwardGPU(core: *RSFCore, x: *Tensor) !bool {
    ...
    const ga = &(core.gpu_accel orelse return false);
    const layer_weights = core.gpu_layer_weights orelse return false;
    if (layer_weights.len != core.num_layers) {
        invalidateGPUForMismatch(core);
        return false;
    }

    var gpu_data = accel.FutharkArray1DF32.newFromSlice(&ga.ctx, x.data) catch {
        invalidateGPUForMismatch(core);
        return false;
    };

    for (layer_weights) |*lw| {
        const output = accel.forwardLayerGPU(&ga.ctx, gpu_data, lw, &ga.dims_arr) catch {
            gpu_data.free(&ga.ctx);
            invalidateGPUForMismatch(core);
            return false;
        };
        gpu_data.free(&ga.ctx);
    ...
    gpu_data.valuesInto(&ga.ctx, x.data) catch {
        gpu_data.free(&ga.ctx);
        invalidateGPUForMismatch(core);
        return false;
    };
